	...
	document.getElementById("modalForEdit").addEventListener("show.bs.modal", function(event){
		...
		fetch(url)
			...
			.then(user => {
				...
				document.getElementById("edit_input_email").value = user.email;
		...
		button.addEventListener("click", function(event){
			...
			data.append("email", document.getElementById("edit_input_email").value);
			...
			fetch(url, {method: "PATCH",
				...
				.then(user => {
					...
					trToEdit.getElementsByClassName("td_email")[0].innerText = user.email;
	...
	document.getElementById("modalForDelete").addEventListener("show.bs.modal", function(event){
		...
		fetch(url)
			...
			.then(user => {
				...
				document.getElementById("delete_input_email").value = user.email;
...
function funcAddNewUserButton(){
	...
	data.append("email", document.getElementById("add_input_email").value);
	...
	.then(user => {
		...
		let td;
		let textNode, form, button, input;
		...
		textNode = document.createTextNode(user.email);
		td = document.createElement("td");
		td.classList.add("td_email");
		td.appendChild(textNode);
		tr.appendChild(td);
